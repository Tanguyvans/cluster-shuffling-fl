...
class GradientReconstructor():
    ...
    def __init__(self, model, device, mean_std=(0.0, 1.0), config=DEFAULT_CONFIG, num_images=1, G=None, bn_prior=((0.0, 1.0)) ):
        ...
        if G:
            ...
            if self.config['generative_model'] == 'stylegan2':
                ...
                G = G.requires_grad_(True)
                self.G = G
            elif self.config['generative_model'] in ['stylegan2_ada']:
                self.G = porting.load_decoder_stylegan2_ada(self.config, self.device, dataset=self.config['gen_dataset'])
    ...
    def init_dummy_z(self, G, generative_model_name, num_images):
        ...
        elif generative_model_name == 'stylegan2_ada':
            # StyleGAN2-ADA uses 512-dimensional latent codes
            dummy_z = torch.randn(num_images, 512).to(self.device).requires_grad_(True)
    ...
    def gen_dummy_data(self, G, generative_model_name, dummy_z, gen_outs=[None], noise=None, ys=None, img_size=-1, start_layer = 0):
        ...
        if generative_model_name == 'stylegan2_ada' or generative_model_name.startswith('stylegan2-ada'):
            # StyleGAN2-ADA with pkl files
            # Ensure correct shape (batch_size, class_label)
            label = torch.zeros([dummy_z.shape[0], G.c_dim], device=dummy_z.device)
            dummy_data = G(dummy_z, label)
    ...
    def reconstruct(self, input_data, labels, img_shape=(3, 32, 32), dryrun=False, tol=None):
        ...
        for idx, label in enumerate(labels_tmp):
            if label in labels:
                indices = torch.nonzero(labels == label).squeeze(-1)
                if indices.dim() == 0:  # Single element
                    infer_labels[indices.item()] = labels_tmp[idx].clone()
                else:  # Multiple elements with same label
                    for ind in indices:
                        if infer_labels[ind.item()] == -1:
                            infer_labels[ind.item()] = labels_tmp[idx].clone()
                            break
        for idx, label in enumerate(labels_tmp):
            if label not in labels:
                if -1 in infer_labels:
                    infer_labels[infer_labels.index(-1)] = labels_tmp[idx].clone()
        infer_labels = torch.stack([l if isinstance(l, torch.Tensor) else torch.tensor(l) for l in infer_labels])            
