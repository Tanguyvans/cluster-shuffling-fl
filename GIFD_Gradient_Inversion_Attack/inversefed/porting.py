...
def load_decoder_stylegan2_ada(config, device, dataset='I128'):
    ...
    if dataset.startswith('I'):
        ...
        # Check for StyleGAN2-ADA CIFAR-10 checkpoint
        import pathlib
        base_path = pathlib.Path(__file__).parent
        # Primary location: stylegan2_ada_pytorch folder
        cifar10_path = base_path / 'genmodels' / 'stylegan2_ada_pytorch' / 'cifar10.pkl'
        if cifar10_path.exists():
            network_pkl = str(cifar10_path)
        else:
            # Fallback to default name
            network_pkl = str(base_path / 'genmodels' / 'stylegan2_ada_pytorch' / 'cifar10u-cifar-ada-best-fid.pkl')
