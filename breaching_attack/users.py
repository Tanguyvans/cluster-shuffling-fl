...
class UserSingleStep(torch.nn.Module):
    ...
    def compute_local_updates(self, server_payload, custom_data=None):
        ...
        if "labels" in data:
            data["labels"] = data["labels"].to(self.setup["device"])
...
class UserMultiStep(UserSingleStep):
    ...
    def compute_local_updates(self, server_payload):
        ...
        if "labels" in user_data:
            user_data["labels"] = user_data["labels"].to(self.setup["device"])
        ...
        shared_grads = [
            (p_local.to(**self.setup) - p_server.to(**self.setup)).clone().detach()
